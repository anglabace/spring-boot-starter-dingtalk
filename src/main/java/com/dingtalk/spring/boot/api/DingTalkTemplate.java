...
public class DingTalkTemplate {
	...
	/*
	 @RequestMapping("getAllDeptId")
	    @ResponseBody
	    public OapiDepartmentListResponse getAllDeptId(@RequestBody JSONObject jsonObject) throws Exception {
	        ...
	        request.setFetchChild(jsonObject.getBoolean("fetchChild"));
	        request.setLang(jsonObject.getString("lang"));
	        request.setHttpMethod("GET");
	        OapiDepartmentListResponse response = client.execute(request, accessToken);
	        return response;
	    }

	    @RequestMapping("getDeptIdSimpleUsers")
	    @ResponseBody
	    public OapiUserSimplelistResponse getDeptIdSimpleUsers(@RequestBody JSONObject jsonObject) throws Exception {
	    	String accessToken = dingTalkTemplate.getAccessToken(dingtalkProperties.getAppKey(), dingtalkProperties.getAppSecret());
	        DingTalkClient client = new DefaultDingTalkClient("https://oapi.dingtalk.com/user/simplelist");
	        OapiUserSimplelistRequest request = new OapiUserSimplelistRequest();
	        request.setDepartmentId(jsonObject.getLong("departmentId"));
	        request.setOffset(jsonObject.getLong("offset"));
	        request.setSize(jsonObject.getLong("size"));
	        request.setHttpMethod("GET");
	        request.setLang(jsonObject.getString("lang"));
	    ...
	    @RequestMapping("testinformSend")
	    @ResponseBody
	    public Object informSend(@Valid @RequestBody InformSendVo informSendVo) throws Exception {
	        Object o = dingTalkProcessService.testFeign(informSendVo);
	        return o;
	    }

	    @RequestMapping("getprocessinstance")
	    @ResponseBody
	    public OapiProcessinstanceGetResponse getprocessinstance(@RequestBody JSONObject jsonObject) throws Exception {
	    	String accessToken = dingTalkTemplate.getAccessToken(dingtalkProperties.getAppKey(), dingtalkProperties.getAppSecret());
	        DingTalkClient client = new DefaultDingTalkClient("https://oapi.dingtalk.com/topapi/processinstance/get");
	        OapiProcessinstanceGetRequest request = new OapiProcessinstanceGetRequest();
	        request.setProcessInstanceId(jsonObject.getString("processInstanceId"));
	        OapiProcessinstanceGetResponse response = client.execute(request,accessToken);
	        logger.info("查看单个审批实例response:{}",JSONObject.toJSONString(response));
	        return response;
	    }

	    @RequestMapping("getprocessinstancelist")
	    @ResponseBody
	    public OapiProcessinstanceListidsResponse getprocessinstancelist(@RequestBody JSONObject jsonObject) throws Exception {
	    	String accessToken = dingTalkTemplate.getAccessToken(dingtalkProperties.getAppKey(), dingtalkProperties.getAppSecret());
	        DingTalkClient client = new DefaultDingTalkClient("https://oapi.dingtalk.com/topapi/processinstance/listids");
	        OapiProcessinstanceListidsRequest req = new OapiProcessinstanceListidsRequest();
	        req.setProcessCode(DingtalkConstant.PROCESS_CODE);
	        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	        req.setStartTime(format.parse("2019-04-01 00:00:00").getTime());
	        req.setEndTime(format.parse("2019-05-31 00:00:00").getTime());
	        req.setSize(10L);
	        req.setCursor(jsonObject.getLong("cursor"));
	        req.setUseridList("061955121419944345,086455061932749508,1348221931648185");
